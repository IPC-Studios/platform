-- A CRM workflow's severity, cooldown and notify routing did nothing.
--
-- 0105 added severity / cooldown_hours / notify_assignee / notify_roles to
-- crm_workflows and seeds seven default rules with real values on them —
-- 'critical' for a hot lead with no follow-up, a 2-hour cooldown on an
-- uncontacted new lead, routing to admin and manager. Nothing ever read any of
-- the four. The action runner notified only the lead's assignee, always at
-- severity 'info', with a dedupe key pinned to now()::date — one per calendar
-- day, whatever the cooldown said.
--
-- So the seeded 2h, 72h and 168h cooldowns all behaved as 24h, the critical
-- rule looked identical to the informational ones in the bell, and a rule
-- routed to managers reached nobody but the assignee.
--
-- Only the notify branch of crm_workflow_do_action changes; every other action
-- is carried across from 0047 untouched.
